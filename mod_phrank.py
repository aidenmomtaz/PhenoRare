...
DISEASE_TO_PHENO="/Users/nadia.momtaz/Downloads/rare_diseases.txt"
XML = "/Users/nadia.momtaz/Downloads/en_product4.xml"
DAG="/Users/nadia.momtaz/Downloads/phrank/demo/data/hpodag.txt"
DISEASE_TO_GENE="/Users/nadia.momtaz/Downloads/phrank/demo/data/gene_to_disease.build127.txt"
GENE_TO_PHENO="/Users/nadia.momtaz/Downloads/phrank/demo/data/gene_to_pheno.amelie.txt"
...
patient_phenotypes2 = ['HP:0001250']
...
@app.post("/process")
def process_inputs(request: PhenotypeRequest):
    d1, d2, d3, d4, d5, s1, s2, s3, s4, s5 = top_disease(request.phenotypes)
    print(s5)
    return {"d1": d1, "d2": d2, "d3": d3, "d4": d4, "d5": d5, "s1": s1, "s2": s2, "s3": s3, "s4": s4, "s5": s5,}
# Calculate phenotype match scores and output the top 5 highest scores and their corresponding disease
def top_disease(phenotypes):
    ...
    print(disease_similarities[-5][0])
    return [disease_similarities[-5][0], disease_similarities[-4][0], disease_similarities[-3][0],
            disease_similarities[-2][0], disease_similarities[-1][0], round(disease_similarities[-5][1],2), round(disease_similarities[-4][1],2), round(disease_similarities[-3][1],2),
            round(disease_similarities[-2][1],2), round(disease_similarities[-1][1],2)]
     

if __name__ == "__main__":
    uvicorn.run(app, host="0.0.0.0", port=8000)

# print(top_disease(patient_phenotypes2))
